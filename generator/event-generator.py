...
from messages_pb2 import GreetRequest, GreetResponse, SeenCount
...
def random_requests():
    ...
    while True:
        ...
        request.val = random.randint(1, 101)
        yield request
...
def produce():
    ...
    for request in random_requests():
        ...
        producer.send(topic='names', key="vals".encode('utf-8'), value=val)
        producer.flush()
...
def consume():
    ...
    for message in consumer:
        ...
        print("count: %s, sum: %s, avg: %s" % (response.seen, response.sum, float(response.sum)/response.seen), flush=True)
